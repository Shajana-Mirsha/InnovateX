...
const SimilarityReviewPage = () => {
  ...
  const loadSimilarityData = async (id) => {
    ...
    setLoading(true);
    try {
      const [hRes, labelsRes, subsRes] = await Promise.all([
        getHackathonById(id),
      ...
      (subsRes.submissions || []).forEach((sub) => {
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="flex items-center gap-3 w-full sm:w-64">
              <span className="text-xs font-mono text-slate-500 font-medium">0.50</span>
              <input
                type="range"
                min="0.5"
                max="0.95"
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 border border-amber-200 text-amber-700 font-mono text-xs font-bold">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-slate-500">
                            Team: <strong className="text-slate-800">{subA.team?.name || subA.team || "N/A"}</strong>
                          </span>
                        </div>
                        <h4 className="text-sm font-bold text-slate-900 tracking-tight">{subA.title}</h4>
                        ...
                        {subA.githubLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-xs text-brand-600 hover:text-brand-700 font-medium pt-1"
                          >
                      ...
                      <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-slate-500">
                            Team: <strong className="text-slate-800">{subB.team?.name || subB.team || "N/A"}</strong>
                          </span>
                        </div>
                        <h4 className="text-sm font-bold text-slate-900 tracking-tight">{subB.title}</h4>
                        <p className="text-xs text-slate-600 line-clamp-4 leading-relaxed">
                          {subB.description}
                        </p>
                        {subB.githubLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700 font-medium pt-1"
                          >
                            View Repository <ExternalLink className="w-3.5 h-3.5" />
                          </a>
                        )}
