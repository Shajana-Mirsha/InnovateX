    ...
    try {
      ...
      if (data.success) {
        ...
        const events = (data.hackathons || []).filter(
          (h) => user.role === "admin" || String(h.createdBy?._id || h.createdBy) === currentUserId
        );
      ...
      <div className="space-y-6 max-w-3xl mx-auto pb-12">
        <div>
          <button
            ...
            className="text-xs font-semibold text-slate-500 hover:text-slate-900 transition inline-flex items-center gap-1"
          >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {formSuccess && (
              <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-700 font-medium">
                {formSuccess}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="block w-full text-sm font-medium text-slate-900 bg-white border border-slate-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition"
                  value={title}
            ...
            <div>
              ...
              <textarea
                ...
                className="block w-full text-sm font-medium text-slate-900 bg-white border border-slate-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 resize-none leading-relaxed transition"
                value={description}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                  <th className="px-6 py-3.5">Title & Domain</th>
                  <th className="px-6 py-3.5">Timeline</th>
                  <th className="px-6 py-3.5">Rubric Dimensions</th>
                  <th className="px-6 py-3.5">Status</th>
                  <th className="px-6 py-3.5 text-right">Research & Management Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-sm">
                {myHackathons.map((hack) => (
                  <tr key={hack._id} className="hover:bg-slate-50/70 transition">
                    <td className="px-6 py-4">
                      <p className="text-sm font-bold text-slate-900">{hack.title}</p>
                      <p className="text-xs text-brand-600 mt-0.5 capitalize font-medium">Domain: {hack.domain}</p>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-lg bg-brand-50 border border-brand-200 text-brand-700 hover:bg-brand-100 transition"
                      >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex gap-2 justify-end items-center flex-wrap">
                        ...
                        >
                          <button className="px-2.5 py-1 text-xs font-medium bg-brand-50 hover:bg-brand-100 border border-brand-200 text-brand-700 rounded-lg transition inline-flex items-center gap-1 shadow-sm">
                        ...
                        >
                          <button className="px-2.5 py-1 text-xs font-medium bg-sky-50 hover:bg-sky-100 border border-sky-200 text-sky-700 rounded-lg transition inline-flex items-center gap-1 shadow-sm">
                        ...
                        >
                          <button className="px-2.5 py-1 text-xs font-medium bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 rounded-lg transition inline-flex items-center gap-1 shadow-sm">
                        ...
                        >
                          <button className="px-2.5 py-1 text-xs font-medium bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 rounded-lg transition inline-flex items-center gap-1 shadow-sm">
                        ...
                        <Link to={`/manage/hackathons?edit=${hack._id}`} title="Edit Settings">
                          <button className="p-1.5 border border-slate-200 text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-100 rounded-lg transition shadow-sm">
